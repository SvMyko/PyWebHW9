...
def run_multiprocessing(start_page, end_page):
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        results = pool.map(parse_quotes, range(start_page, end_page + 1))
    indexed_results = list(enumerate(results, start=start_page))
    indexed_results.sort()
    quotes = []
    authors = []
    for _, result in indexed_results:
        quotes.extend(result[0])
        authors.extend(result[1])
    return quotes, authors
...
def main():
    while True:
        try:
            ...
            quotes, authors = run_multiprocessing(start_page, end_page)
